...
const Home = ({ navigation }) => {
  ...
  const openTicketsCount = user && user.ticket.filter(item => item.currentStatus.Open === "Open").length;
  const closedTicketsCount = user && user.ticket.filter(item => item.currentStatus.Open === "Closed").length;

  const handleSearch = (query) => {
    setSearchQuery(query);

    const results = user && user.ticket.filter(
      (item) =>
        item.customer.name.toLowerCase().includes(query.toLowerCase()) ||
        item.ticket.ticketNumber.toLowerCase().includes(query.toLowerCase()) ||
        item.remarks.serviceType.toLowerCase().includes(query.toLowerCase())
    );
  ...
  return (
    <SafeAreaView style={styles.mainContainer}>
      ...
      <LinearGradient
        ...
        <SearchBar
          platform="ios"
          containerStyle={{ backgroundColor: 'white', width: width / 1.12, alignSelf: 'center', borderRadius: 20, opacity: 0.95, height: 70 }}
      ...
      <View style={{ flex: 1, marginBottom: 50, width: width, alignSelf: 'center', marginTop: 0 }}>
        <FlatList
          ...
          data={searchQuery ? searchResults : user && user.ticket}
          ...
          renderItem={({ item }) => (
            <Ticket
              ...
              Date={"24/02/2022"}
              ServiceType={item.remarks.serviceType}
              CustomerName={item.customer.name}
              Status={item.currentStatus.status}
              Criticality={item.ticket.criticality}
